...
uint8_t diagMeasResult[5+3]; 		// data byte array for cmd8 and index1 return
uint8_t tempNoiseMeasResult[4+3]; 	// data byte array for cmd6 and index0&1 return
...
bool burnEEPROM(int serial_port)
{
	...
	usleep(1000);  // Wait for 100 msecond before sending data again delay(1);
	...
	for(int n=0; n<3; n++)
	{
		if(n==1)
		{
			burnStat = serialGetchar(serial_port); // store EE_CNTRL data
		}
		else
		{
